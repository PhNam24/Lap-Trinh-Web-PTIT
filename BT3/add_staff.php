<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (empty($_POST["email"])) {
            ...
            $email_query = "SELECT * FROM nhan_vien_tbl WHERE email = '$email'";
            $email_result = $sql_connect->query($email_query);
        ...
        if (empty($_POST["phone"])) {
            ...
            $phone_query = "SELECT * FROM nhan_vien_tbl WHERE so_dien_thoai = '$phone'";
            $phone_result = $sql_connect->query($phone_query);
    ...
    if (isset($_POST['submit'])) {
        if (!empty($name) && !empty($email)  && !empty($avatar) && !empty($birth) && !empty($gender) && !empty($address) && !empty($phone) && !empty($department) && !empty($join) && checkErr($nameErr, $emailErr, $genderErr, $addressErr, $phoneErr)) {
            ...
            $sql_connect->query($query2);

            $id_query = "SELECT id_nv FROM nhan_vien_tbl WHERE email = '$email'";
            $id_result = $sql_connect->query($id_query);
            $id_new = 0;
            if($row = $id_result->fetch_assoc()) {
                $id_new = $row['id_nv'];
            }

            $query3 = "INSERT INTO login_tbl(id_login, username, password, user_type, id_nv) VALUES (NULL,'$email','$phone',2, $id_new);";

            if ($sql_connect->query($query3)) {
                header("Location: staff.php");
                exit();
            }
            else {
                header("Location: index.html?error=$sql_connect->error");
                exit();
            }
